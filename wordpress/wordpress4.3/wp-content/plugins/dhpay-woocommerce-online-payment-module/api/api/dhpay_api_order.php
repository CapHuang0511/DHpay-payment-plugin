 ...
 * @copyright Copyright (c) 2011-2012, DHPAY
 */
class Dhpay_Order_Helper {
 ...
 * You can add as many products as you want, just remember that the total amount for the products must match the total amount of the Dhpay Payment Object
 *  
 * @version 1.0.0
 * 
 * @author Wouter van Tilburg 
...
class Dhpay_Order_Product {
    ...
    /**
     * Creates and returns a new Dhpay_Order_Product
     * 
     * @since 1.0.0     
     * @return \Dhpay_Order_Product
     */
...
class Dhpay_Order_Address {
    ...
    /**
     * Creates and returns a new Dhpay_Order_Address
     * 
     * @since 1.0.0     
     * @return \Dhpay_Order_Address
     */
    public static function create() {
        return new self();
    }
...
class Dhpay_Order {
     ...
     * 
     * @since 1.0.0
     * @param obj Object containing the Dhpay_Order_Consumer class
     * @return \Dhpay_Order
     */
    public function setConsumer(Dhpay_Order_Consumer $obj) {
     ...
     * 
     * @since 1.0.0
     * @param obj Object containing the Dhpay_Order_Address class
     * @return \Dhpay_Order
     */
    public function setShippingAddress(Dhpay_Order_Address $obj) {
     ...
     * @return \Dhpay_Order
     */
    public function setBillingAddress(Dhpay_Order_Address $obj) {
     ...
     * 
     * @since 1.0.0
     * @param obj object containing the Dhpay_Order_Product class
     * @return \Dhpay_Order
     */
    public function addProduct(Dhpay_Order_Product $obj) {
    ...
    public function setOrderDiscountAmount($amount, $name = 'Discount', $description = 'Order Discount') {
        $obj = Dhpay_Order_Product::create()
                ->setProductID('02')
                ->setProductName($name)
                ->setDescription($description)
                ->setQuantity('1')
                ->setUnitPrice(-$amount)
                ->setVATCategory(Dhpay_Order_VAT::getCategoryForPercentage(-1));
    ...
    public function setShippingCosts($amount, $vat = -1, $name = 'Shipping Costs') {
        $obj = Dhpay_Order_Product::create()
                ...
                ->setQuantity('1')
                ->setUnitPrice($amount)
                ->setVATCategory(Dhpay_Order_VAT::getCategoryForPercentage($vat));
    ...
    public function validateOrder($paymentObj) {
        switch (strtoupper($paymentObj->getPaymentMethod())) {
            case 'AFTERPAY':
                ...
                if (!Dhpay_Order_Helper::validateZipCode($this->_data['shippingAddress']->zipCode, $this->_data['shippingAddress']->country))
                    throw new Exception('Zipcode format for shipping address is incorrect.');

                if (!Dhpay_Order_Helper::validateZipCode($this->_data['billingAddress']->zipCode, $this->_data['billingAddress']->country))
                    throw new Exception('Zipcode format for billing address is incorrect.');

                if (!Dhpay_Order_Helper::validatePhonenumber($this->_data['consumer']->phone))
