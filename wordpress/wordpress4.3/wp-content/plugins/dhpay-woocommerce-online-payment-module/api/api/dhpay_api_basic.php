 ...
 *
 */
class Dhpay_Api_Basic extends Dhpay_Api_Base {
            ...
            if ($handle) {
                while (false !== ($file = readdir($handle))) {
                    if ($file != "." && $file != ".." && $file != ".svn") {
                        ...
                        $className = "Dhpay_Paymentmethod_" . ucfirst($name);
 ...
 * 
 * <pre>
 * $paymentObj = new Dhpay_PaymentObject();
 * $paymentObj->setPaymentMethod('IDEAL')
 *            ->setAmount(1000)
 *            ->setCountry("NL")
 *            ->setLanguage("NL")
 *            ->setReference("DHPAY Test Payment")
 *            ->setDescription("DHPAY Test Payment")
 *            ->setCurrency("EUR")
 *            ->setIssuer('ING')
 *            ->setOrderID('icetest01');
 * 
 * $basicmode = Dhpay_Basicmode::getInstance();
 ...
 */
class Dhpay_Basicmode extends Dhpay_Api_Base {

    private static $instance;
    protected $_basicmodeURL = "pay.dhpay.eu/basic/";
     ...
     * @since version 2.1.0
     * @access public
     * @param Dhpay_PaymentObject_Interface_Abstract $payment
     */
    public function validatePayment(Dhpay_PaymentObject_Interface_Abstract $payment) {
    ...
    /**
     * Post the fields and return the URL generated by DHPAY
     * @since version 1.0.0
     * @access public
    ...
    public function getURL() {
        ...
        /* @since version 1.0.2 */
        if ($this->_webservice) {
            require_once(DIR . DS . "dhpay_api_webservice.php");

            if (!isset($this->data->ic_issuer) || $this->data->ic_issuer == "")
                throw new Exception("Issuer not set");

            //$ws = Dhpay_Api_Webservice::getInstance()->paymentService();
            $ws = Dhpay_Api_Webservice::getInstance()->paymentService();
    ...
    /**
     * Generates a URL to the DHPAY basic API service
     * @since version 1.0.0
     * @access protected
     * @return string URL
     */
    ...
    /**
     * Used to connect to the DHPAY servers
     * @since version 1.0.0
     * @access protected
     * @param string $url
     * @param array $data
