...
// Include API base 
require_once(realpath(dirname(__FILE__)) . DIRECTORY_SEPARATOR . 'dhpay_api_base.php');
 ...
 * @copyright Copyright (c) 2013, DHPAY
 */
interface Dhpay_PbmObject_Interface_Abstract {
 ...
 *
 */
class Dhpay_Api_Pbm extends Dhpay_Api_Base {

    private $url = 'http://pbm.dhpay.com/api';
     ...
     * 
     * @since 1.0.0
     * @param Dhpay_Pbm_Object $pbmObject
     * @return string
     */
    public function createLink(Dhpay_Pbm_Object $pbmObject)
    {
    ...
    private function validateSettings()
    {
        ...
        if (!Dhpay_Parameter_Validation::merchantID($this->getMerchantID()))
            throw new Exception('Merchant ID not set, use the setMerchantID() method', 1001);

        // Validate SecretCode
        if (!Dhpay_Parameter_Validation::secretCode($this->getSecretCode()))
 ...
 *
 */
class Dhpay_Pbm_Object implements Dhpay_PbmObject_Interface_Abstract {
    ...
    public function setCurrency($currency)
    {
        if (!Dhpay_Parameter_Validation::currency($currency))
    ...
    {
        if (!Dhpay_Parameter_Validation::country($countryCode))
    ...
    public function setLanguage($language)
    {
        if (!Dhpay_Parameter_Validation::language($language))
    ...
    public function setOrderID($orderID)
    {
        if (!Dhpay_Parameter_Validation::orderID($orderID))
