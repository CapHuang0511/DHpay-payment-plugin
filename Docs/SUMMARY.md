# 目录

## DHpay GateWay文档
* [DHpay集成文档](README.md)
    * [版本](history-version.md)
    * [保密和使用](usage.md)
    * [DHpay介绍](dhpay-intro.md)
* [DHpay GateWay 集成文档](dhpay-gateway-integrated.md)
    * [信用卡支付流程图](credit-card-payment-process.md)
    * [功能演示](functional-demonstration.md)
    * [集成步骤](integrate-steps.md)
        * [请求地址](interface-of-payment-page.md)
        * [查询 Api](query-api.md)
        * [接入方式](embed-page-interface.md)
        * [开启Token快捷支付](token-quick-payment.md)
    * [常见错误](exception-handling.md)
* Q & A
    * [DHpay 提供测试环境么](does-dhpay-have-test-env.md)
    * [DHpay 支持哪些交易币种](currency-supported.md)
    * [invoice\_id、order\_no、ref\_no 有什么区别](different-between-invoice-order-ref-id.md)
    * [商户可以对自己的交易关闭 3D 认证功能么](could-3d-shut-by-merchant-self.md)
    * [商户可以存储客户的信用卡信息么](could-cardinfo-saved-by-merchant.md)
    * [为什么我使用国内发行的卡片上同时有 VISA 和银联标 志的信用卡不能支付成功?](why-visacard-in-china-pay-failed.md)
    * [在集成期间,如果遇到问题,我们如何联系?](how-to-contact-us.md)
* 附录
    * [错误响应码](error-code.md)
    * [错误原因引导提示](error-reason-and-guide.md)
    * [国家代码](country-code.md)
    * [签名算法](sign-algorithm.md)
    * [PHP 版本接入示例](php-access-example.md)
    * [ASP 版本接入示例](asp-access-version.md)
    * [测试信用卡信息](test-card-informations.md)

