 ...
 */
class Idev_Dhpay_Block_Form extends Mage_Payment_Block_Form
{
    /**
    ...
    {
        if ($payment == 'dhpay_obt') {
            $payment .= '_'.$this->getInfoLocale();
        }
    
        $imageFilename = Mage::getDesign()
            ->getFilename('images' . DS . 'dhpay' . DS . $payment, array('_type' => 'skin'));

        if (file_exists($imageFilename . '.png')) {
            return $this->getSkinUrl('images/dhpay/' . $payment . '.png');
        } else if (file_exists($imageFilename . '.gif')) {
            return $this->getSkinUrl('images/dhpay/' . $payment . '.gif');
        }
    ...
    {
        $locale = substr(Mage::app()->getLocale()->getLocaleCode(), 0 ,2);
        return 'http://www.dhpay.com/app/?l=' . strtoupper($locale);
    }
}
