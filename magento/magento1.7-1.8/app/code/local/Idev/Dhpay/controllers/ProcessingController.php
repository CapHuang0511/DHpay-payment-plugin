 ...
 */
class Idev_Dhpay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    /**
     * Iframe page which submits the payment data to Dhpay.
     */
    ...
    /**
     * Show orderPlaceRedirect page which contains the Dhpay iframe.
     */
    public function paymentAction()
    {
        try {
            ...
            $order->setState(Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                Mage::helper('dhpay')->__('The customer was redirected to Dhpay.')
            );
            $order->save();

            $session->setDhpayQuoteId($session->getQuoteId());
            $session->setDhpayRealOrderId($session->getLastRealOrderId());
    ...
    {
        $event = Mage::getModel('dhpay/event')
    ...
    public function cancelAction()
    {
        ...
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getDhpayQuoteId()) {
